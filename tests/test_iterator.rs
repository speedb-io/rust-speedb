...
use speedb::{Direction, IteratorMode, MemtableFactory, Options, DB};
...
fn test_prefix_iterator() {
    let n = DBPath::new("_rust_rocksdb_prefix_iterator_test");
    {
        ...
        let prefix_extractor = speedb::SliceTransform::create_fixed_prefix(3);
...
fn test_prefix_iterator_uses_full_prefix() {
    ...
    let path = DBPath::new("_rust_rocksdb_prefix_iterator_uses_full_prefix_test");
    {
        ...
        let prefix_extractor = speedb::SliceTransform::create_fixed_prefix(1);
...
fn test_iter_range() {
    ...
    fn test(
        db: &DB,
        mode: IteratorMode,
        range: impl speedb::IterateBounds,
        want: std::ops::Range<usize>,
        reverse: bool,
    ) {
        let mut ro = speedb::ReadOptions::default();
    ...
    fn prefix(key: &[u8]) -> speedb::PrefixRange<&[u8]> {
        speedb::PrefixRange(key)
    }
